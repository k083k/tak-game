...
function getOrCreatePlayerId() {
  let id = localStorage.getItem('tac_player_id');
  if (!id) {
    id = crypto.randomUUID();
    localStorage.setItem('tac_player_id', id);
  }
  return id;
}
...
export const MultiplayerService = {
  ...
  async createGame(playerName, avatar) {
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
      code = generateCode();
      const { data } = await supabase.from('tac_games').select('id').eq('code', code).maybeSingle();
      if (!data) break;
    }
  ...
  subscribeToPlayers(gameId, callback) {
    const channel = supabase
      .channel(`tac_players:${gameId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'tac_game_players', filter: `game_id=eq.${gameId}` }, callback)
      .subscribe();
  ...
  subscribeToGameState(gameId, callback) {
    const channel = supabase
      .channel(`tac_state:${gameId}`)
      .on('postgres_changes', { event: '*', schema: 'public', table: 'tac_game_state', filter: `game_id=eq.${gameId}` },
